...
public class ThrallManager {
    ...
    /** All online players who are thralls bound to the given master. */
    public java.util.List<Player> getOnlineThralls(UUID masterId) {
        java.util.List<Player> out = new java.util.ArrayList<>();
        for (Player p : Bukkit.getOnlinePlayers()) {
            ThrallBondData d = bonds.get(p.getUniqueId());
            if (d != null && d.isBoundTo(masterId)) out.add(p);
        }
        return out;
    }
    ...
    private void applyBloodBottleMode(Player thrall, ThrallBondData d, UUID ownerId, boolean isM1) {
        ...
        if (newStage > prevStage) {
            if (plugin.getNetwork() != null && plugin.getNetwork().isEnabled()) {
                String masterName = Bukkit.getOfflinePlayer(ownerId).getName();
                plugin.getNetwork().logEvent(
                        "thrall_bond_stage_" + newStage,
                        thrall.getUniqueId().toString(),
                        thrall.getName(),
                        "master=" + (masterName != null ? masterName : ownerId.toString()) + ",stage=" + newStage);
                plugin.getNetwork().updatePlayer(
                        thrall.getUniqueId().toString(),
                        thrall.getName(),
                        "thrall_stage" + newStage,
                        null);
            }
    ...
    private int getCooldownSeconds(String key) {
        switch (key) {
            ...
            case "human_blood":  return config.getHumanBloodDrawCooldownSeconds();
            case "animal_blood": return config.getAnimalBloodDrawCooldownSeconds();
            default:             return 60;
        }
    }
